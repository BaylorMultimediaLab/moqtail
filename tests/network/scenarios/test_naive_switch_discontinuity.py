...
behind-live (filtered) client using the relay's 0-sentinel "live-edge" SWITCH
...
ordinary floor (oldest common boundary), and live-edge mode now expresses
...
async def test_naive_switch_on_filtered_client_is_gap_free(
    ...
    """Filtered client with delay=2s, live-edge switch (conformant floor-at-latest).
    ...
    # floor-at-latest live-edge mode the new track re-delivers from the client's
    ...
    assert abs(pts_gap) <= MAX_SEAM_GAP_MS, (
        f"expected gap-free live-edge switch (floor at latest received group, "
    ...
    print(
        f"[C7] conformant live-edge switch max ptsGapMs={pts_gap}ms across "
