"""Slice B verification: filtered+time-shifted switch eliminates the discontinuity.

Mirror of test_naive_switch_discontinuity (Slice C). Same harness, same
ABR-driven natural switches, but with switchMode='time-shifted' the relay
...
If time-shifted switching is correct, ptsGapMs should be tightly bounded at the
GOP boundary (positive or negative under one GOP). If it falls back to the
live-edge path (TimeMap miss, or relay didn't honor the parameter), ptsGapMs
...
async def test_aligned_switch_on_filtered_client_eliminates_discontinuity(
    ...
    """Filtered client with delay=2s, time-shifted switch.
    Wait for ABR-driven natural switches; assert ptsGapMs is bounded
    by ~one GOP (much smaller than live-edge's ~delay-magnitude jump).
    """
    page = browser_page
    ...
    # Confirm the records actually came from the time-shifted mode (sanity).
    aligned_records = [r for r in switch_records if r.get("switchMode") == "time-shifted"]
    assert aligned_records, (
        f"expected switchMode='time-shifted' on every switch record. Got modes: "
        f"{[r.get('switchMode') for r in switch_records]}"
    )

    # Headline assertion: time-shifted switch should keep ptsGapMs within one GOP.
    ...
    assert abs(pts_gap) < 1500, (
        f"expected time-shifted switch discontinuity within ~1 GOP, "
        f"got max ptsGapMs={pts_gap}. "
        f"If this is ~2000ms, the time-shifted parameter likely didn't propagate "
    ...
    if len(misses) > 1:
        pytest.fail(
            f"multiple TimeMap misses in time-shifted mode ({len(misses)}/{len(switch_records)}). "
            f"At most one early-startup miss is acceptable. Misses: {misses}"
        )

    print(
        f"[B7] time-shifted switch ptsGapMs={pts_gap}ms across {len(switch_records)} "
        f"switch event(s) (filtered, delay=2s, switchMode=time-shifted)",
        flush=True,
    )
