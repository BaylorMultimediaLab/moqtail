...
function readRunParams() {
  ...
  return {
    ...
    switchFloor:
      params.get('switchFloor') === 'playhead' ? ('playhead' as const) : ('next-group' as const),
  };
}
...
export function App() {
  ...
  // SWITCH PR #1378 floor policy (branch-specific mechanism knob).
  const [switchFloor, setSwitchFloor] = useState<'next-group' | 'playhead'>(runParams.switchFloor);
  ...
  const startPlayback = useCallback(
    ...
    [relayUrl, namespace, disposePlayer, abrSettings, clientMode, filterDelaySeconds, switchFloor],
  );
  ...
  return (
    <div className="flex h-dvh w-dvw flex-col bg-neutral-950 font-sans text-neutral-100 antialiased">
      ...
      <SettingsPanel
        ...
        onSwitchFloorChange={setSwitchFloor}
      />
