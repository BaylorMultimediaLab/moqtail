...
export interface SettingsPanelProps {
  ...
  /** SWITCH PR #1378 Minimum Switching Group ID policy. */
  switchFloor: 'next-group' | 'playhead';
  onSwitchFloorChange: (floor: 'next-group' | 'playhead') => void;
}
      ...
      <div className="relative">
        <div className="flex snap-x snap-proximity scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-transparent gap-3 overflow-x-auto pb-2">
          ...
          <OptionCard title="Connection">
            ...
            <SectionLabel>Switch Floor (PR #1378)</SectionLabel>
            <div className="mb-2 flex gap-1">
              {(['next-group', 'playhead'] as const).map(m => (
                <button
                  key={m}
                  disabled={connectStatus !== 'idle'}
                  onClick={() => onSwitchFloorChange(m)}
                  className={cn(
